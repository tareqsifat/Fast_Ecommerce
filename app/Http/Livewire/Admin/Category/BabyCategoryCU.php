...
class BabyCategoryCU extends Component
{
    ...
    */
    public function getModalId($item)
    {
        ...
        $fineBydata   = BabyCategory::find($itemIdentity);
        $this->name   =  $fineBydata->name;
        $this->sub_sub_category_id   =  $fineBydata->sub_sub_category_id;
        $this->subcategory_id   =  $fineBydata->SubSubCategory->subcategory->id;
    ...
    */
    public function save()
    {
        ...
        if (is_null($this->item)) {
            ...
            // data update 
            BabyCategory::find($this->item)->update([
                'name' => $this->name,
                'slug' => Str::slug($this->slug),
                'sub_sub_category_id' => $this->sub_sub_category_id,
                'image' => $image,
    ...
    */
    ...
    {
        $subcategorys = Subcategory::where('status', 0)->where('shop_as', 'fast_deals')->get();
        $pCatData = SubSubCategory::where('subcategory_id', $this->subcategory_id)->where('status', 0)->where('shop_as', 'fast_deals')->get();
        return view('livewire.admin.category.baby-category-c-u', compact(['pCatData', 'subcategorys']));
    }
}
