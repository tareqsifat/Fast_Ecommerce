...
use App\Models\NewBornCategory as ModelsNewBornCategory;
use Illuminate\Support\Facades\File;
use Livewire\Component;

class NewBornCategory extends Component
{
    ...
    {
        $this->item = $data;
        $newData = ModelsNewBornCategory::find($data);
        if ($action == 'edit') {
    ...
    {
        return view('livewire.admin.category.new-born-category', [
            'datas' => ModelsNewBornCategory::where('name', 'LIKE', "%{$this->searchItem}%")
                ->where('shop_as', 'fast_deals')
                ->orwhere('description', 'LIKE', "%{$this->searchItem}%")
                ->orderBy('id', 'desc')->paginate($this->perPage),
            'alldatas' =>  ModelsNewBornCategory::where('shop_as', 'fast_deals')->get()
        ]);
    }
}
