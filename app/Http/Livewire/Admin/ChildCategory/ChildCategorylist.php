...
class ChildCategorylist extends Component
{
    ...
    {
        $delData = SubSubCategory::find($id);
    ...
    {
        $this->item = $data;
        $newData = SubSubCategory::find($data);
        if ($action == 'edit') {
    ...
    {
        return view('livewire.admin.child-category.child-categorylist', [
            'datas' => SubSubCategory::where('name', 'LIKE', "%{$this->searchItem}%")
                ->where('shop_as', 'fast_deals')
                ->orwhere('description', 'LIKE', "%{$this->searchItem}%")
                ->orderBy('id', 'desc')->paginate($this->perPage),
            'alldatas' =>  SubSubCategory::where('shop_as', 'fast_deals')->get()
        ]);
    }
}
