...
class ShowMerchantProduct extends Component
{
    public $perPage = 25;
    public $searchItem;
    public $item;
    ...
    {
        return view('livewire.admin.product.show-merchant-product', [
            'datas' => Product::where('product_for', 'fast_deals')
                ->where('title', 'LIKE', "%{$this->searchItem}%")
                ->where('shop_for', 'Merchant')
                ->orderBy('id', 'desc')->paginate($this->perPage),
            'alldata' =>  Product::where('product_for', 'fast_deals')->where('shop_for', 'Merchant')->get()
        ]);
    }
}
