(function ($) {
	...
	var MERCADO_JS = {
		init: function () {
			...
			this.mercado_countdown_hotdeals();
			...
			if ($(".mercado-countdown").length > 0) {
				$(".mercado-countdown").each(function (index, el) {
					...
					_this.countdown(_expire, function (event) {
						$(this).html(event.strftime('<small class="font-size-16 mx-2 text_success timer_before_text"> Ends after </small><span>%D d</span> <span>%H h</span> <span>%M m</span> <span>%S s</span>'));
					});
				});
			}
		},
		// hot deals 
		mercado_countdown_hotdeals: function () {
			if ($(".mercado-hote-deals-countdown").length > 0) {
				$(".mercado-hote-deals-countdown").each(function (index, el) {
					var _this = $(this),
						_expire = _this.data('expire');
					_this.countdown(_expire, function (event) {
						$(this).html(event.strftime('<div class="d-flex hotdeals_count_item"><span><p class="d-count">%D</p>  <p>Day</p></span> <span><p class="d-count">%H</p><p>Hours</p></span><span><P class="d-count">%M</P>  <p>Mins</p></span> <span><p class="d-count">%S</p>  <p>Secs</p></span></div>'));
					});
				});
			}
		},
