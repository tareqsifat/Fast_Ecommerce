<div wire:ignore.self class="modal fade" id="befor_born_Cat_Form" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold text-success" id="exampleModalLabel">Before Born Category</h5>
            ...
            <div class="modal-body">
                <div class="card p-3">
                    ...
                    <div class="form-group">
                        <label for="subcategory_id">Baby Category:</label>
                        ...
                        <select wire:model="subcategory_id" id="subcategory_id" class="form-control">
                            ...
                            @foreach($subcategorys as $subitem)
                            <option value="{{$subitem->id}}">{{$subitem->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="sub_sub_category_id">New Born Category:</label>
                        @if($errors->has('sub_sub_category_id'))<span class="text-danger">{{$errors->first('sub_sub_category_id')}}</span> @endif
                        <select wire:model="sub_sub_category_id" id="sub_sub_category_id" class="form-control">
                            <option>--- Select Category ---</option>
                            @foreach($pCatData as $bitem)
