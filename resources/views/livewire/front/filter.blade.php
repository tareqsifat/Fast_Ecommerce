    ...
    <div class="container">
        <div class="wrap-breadcrumb">
            <ul class="my-4">
                ...
                @if(session('search_by') == 'categories')
                <li class="item-link"><a href="{{route('parent-category.single',$cateData->slug)}}" class="link">{{$cateData->name}}</a></li>
                @elseif(session('search_by') == 'subcategories')
                <?php $parentCategoryItem  = DB::table('categories')->where('id', $cateData->category_id)->first(); ?>
                <li class="item-link"><a href="{{route('parent-category.single',$parentCategoryItem->slug)}}" class="link">{{$parentCategoryItem->name}}</a></li>
                <li class="item-link"><a href='{{route("category.single", [$parentCategoryItem->slug, $cateData->slug])}}' class="link">{{$cateData->name}}</a></li>
                @elseif(session('search_by') == 'sub_sub_categories')
                <?php $subCategoryItem  = DB::table('subcategories')->where('id', $cateData->subcategory_id)->first(); ?>
                <?php $parentCategoryItem = DB::table('categories')->where('id', $subCategoryItem->category_id)->first(); ?>
                <li class="item-link"><a href="{{route('parent-category.single', $parentCategoryItem->slug)}}">{{$parentCategoryItem->name}}</a></li>
                <li class="item-link"><a href='{{route("category.single", [$parentCategoryItem->slug, $subCategoryItem->slug])}}' class="link">{{$subCategoryItem->name}}</a></li>
                <li class="item-link"><a href='{{route("childcategory.single", [$parentCategoryItem->slug,$subCategoryItem->slug, $cateData->slug])}}'>{{$cateData->name}}</a></li>
                @endif
            </ul>
        </div>
        <div class="w-100 row m-auto">
            @if($uniquebrads->count()>0)
            <!-- heading  -->
            ...
            <ul class="list-unstyled border_none style-nav-1">
                @foreach($uniquebrads as $key=>$uItem)
                @if($key < 7) <!-- fatcha brand data -->
                    <?php $nData = DB::table('brands')->where('id', $uItem)->first() ?>
                    <!-- fatcha brand data  -->
        ...
        <div class="w-100 row m-auto pb-5 pt-4">
            <div class="row m-auto">
                ...
                <form action="{{route('filter.post',$cateData->slug)}}" class="autoSubmit" method="post">
                    ...
                    <input type="hidden" name="search_by" value="filter">
                                    ...
                                    <div class="form-group searchBox_select mb-0">
                                        <div class="float-right">
                                            ...
                                            <Select name="short_by" class="auto-submit-item">
                                                <option @if(session('short_by')=='desc' ) {{'selected' }} @endif value="desc">Price(high > low)</option>
                                                <option @if(session('short_by')=='asc' ) {{'selected' }} @endif value="asc">Price(low > high)</option>
                                            </Select>
                                        </div>
