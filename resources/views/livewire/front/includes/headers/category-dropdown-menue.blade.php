    <div class="col-lg-3 col-sm-12 col-md-3 col-xs-12 p-xsm-0 pl-0 visibility-hidden sitebar">
        <div class="widget mercado-widget categories-widget">
            <div class="widget-content display_none d-none d-md-none d-lg-block">
                <ul class="list-category mt-2 nav primary clone-main-menu" id="category" data-menuname="Category">
                    ...
                    <li class="category-item has-child-cate border_bottom_eee p-0 bg_white categorymain-item">
                        <a href="{{route('parent-category.single', $catItem->slug)}}">
                            <div class="row d-flex">
                                <div class="col-md-10 col-sm-10 col-xs-10 col-xsm-10 pr-0">
                                    <span class="cate-link">{{ucfirst($catItem->name)}} @if($catItem->product->count() > 0) <span class="text-info">({{$catItem->product->count()}})</span> @endif</span>
                                </div>
    ...
    <script>
        $('.sitebar').hide();
        $('.menue_show').click(function() {
            $('.sitebar').removeClass('visibility-hidden');
            $('.sitebar').toggle();
        });
        $('#main, .topbar-menu-area, .dropdown_menue_action1').click(function() {
            $('.sitebar').hide();
        })
    </script>
    @endpush