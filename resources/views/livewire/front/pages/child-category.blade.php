    ...
    <div class="container">
        <div class="wrap-breadcrumb">
            <ul class="my-4">
                ...
                <?php $catId = $cateData->subcategory->category_id;
                $pCategory = DB::table('categories')->where('id', $catId)->first(); ?>
                <li class="item-link"><a href="{{route('parent-category.single',$pCategory->slug)}}" class="link">{{$pCategory->name}}</a></li>
                <li class="item-link"><a href='{{route("category.single", [$pCategory->slug, $cateData->subcategory->slug])}}' class="link">{{$cateData->subcategory->name}}</a></li>
                <li class="item-link"><span class="text_success">{{$cateData->name}}</span></li>
            </ul>
        </div>
        <div class="w-100 row m-auto">
            @if($cateData->babycategory->count()>0)
            <ul class="list-unstyled border_none style-nav-1 categoryImage-slider owl-carousel">
                @foreach($cateData->babycategory as $key=>$uItem)
                <li class="">
                    <div class="wrap-show-advance-info-box style-1 w-100 mb-0 ">
                        <div class="wrap-products">
                            <a href='{{route("baby_category.single", [$cateData->subcategory->categories->slug, $cateData->subcategory->slug,$cateData->slug, $uItem->slug])}}' title="{{$uItem->name}}">
                                <figure>
                            ...
                            <div class="product-info">
                                <h5 class="font-weight600 text-align-center mt-5 mb-5 text-uppercase"><a class="text_dark" href='{{route("baby_category.single", [$cateData->subcategory->categories->slug, $cateData->subcategory->slug, $cateData->slug,$cateData->slug, $uItem->slug])}}'> {{Str::of($uItem->name)->words(2, '')}}</a> </h5>
                            </div>
                        </div>
                    </div>
                </li>
