    ...
    <div class="container">
        <div class="wrap-breadcrumb">
            <ul class="my-4">
                ...
                <li class="item-link"><a href="{{route('parent-category.single',$cateData->babyCategory->SubSubCategory->subcategory->categories->slug)}}" class="link">{{$cateData->babyCategory->SubSubCategory->subcategory->categories->name}}</a></li>
                <li class="item-link"><a href='{{route("category.single", [$cateData->babyCategory->SubSubCategory->subcategory->categories->slug, $cateData->babyCategory->SubSubCategory->subcategory->slug])}}' class="link">{{$cateData->babyCategory->SubSubCategory->subcategory->name}}</a></li>
                <li class="item-link"><a href='{{route("childcategory.single", [$cateData->babyCategory->SubSubCategory->subcategory->categories->slug, $cateData->babyCategory->SubSubCategory->subcategory->slug, $cateData->babyCategory->SubSubCategory->slug])}}' class="link">{{$cateData->babyCategory->SubSubCategory->name}}</a></li>
                <li class="item-link"><a href='{{route("baby_category.single", [$cateData->babyCategory->SubSubCategory->subcategory->categories->slug, $cateData->babyCategory->SubSubCategory->subcategory->slug, $cateData->babyCategory->SubSubCategory,$cateData->babyCategory->slug])}}' class="link">{{$cateData->babyCategory->name}}</a></li>
                <li class="item-link"><span class="text_success">{{$cateData->name}}</span></li>
            </ul>
        </div>
        <div class="w-100 row m-auto">
            @if($cateData->beforeBornCategory->count()>0)
            <ul class="list-unstyled border_none style-nav-1 categoryImage-slider owl-carousel">
                @foreach($cateData->beforeBornCategory as $key=>$uItem)
                <li class="">
                    <div class="wrap-show-advance-info-box style-1 w-100 mb-0 ">
                        <div class="wrap-products">
                            <a href='{{route("beforenewborn_category.single", [$cateData->babyCategory->SubSubCategory->subcategory->categories->slug, $cateData->babyCategory->SubSubCategory->subcategory->slug, $cateData->babyCategory->SubSubCategory,$cateData->babyCategory->slug, $cateData->slug, $uItem->slug])}}' title="{{$uItem->name}}">
                                <figure>
                                    <img style="border: 1px solid #e2e2e2;border-radius: 50%;height: 100px;width: 100px; margin:auto;" src='@if($uItem->image) @if(file_exists("uploads/category/$uItem->image")){{asset("uploads/category/$uItem->image")}} @else {{asset("defaults/default-blank-img.jpg")}}  @endif  @else {{asset("defaults/default-blank-img.jpg")}} @endif' alt="{{$uItem->brand_logo}}">
                                </figure>
                            </a>
                            <div class="product-info">
                                <h5 class="font-weight600 text-align-center mt-5 mb-5 text-uppercase"><a class="text_dark" href='{{route("beforenewborn_category.single", [$cateData->babyCategory->SubSubCategory->subcategory->categories->slug, $cateData->babyCategory->SubSubCategory->subcategory->slug, $cateData->babyCategory->SubSubCategory,$cateData->babyCategory->slug, $cateData->slug, $uItem->slug])}}'> {{Str::of($uItem->name)->words(2, '')}}</a> </h5>
                            </div>
                        </div>
                    </div>
                </li>
        ...
        <div class="w-100 row m-auto pb-5 pt-4">
            <div class="row m-auto">
                ...
                <form action="{{route('filter.post', $cateData->slug)}}" class="autoSubmit" method="POST">
                    ...
                    <input type="hidden" name="search_by" value="sub_sub_categories">
                    ...
                    @if(isset($fProduct) && $fProduct->count()>0)
